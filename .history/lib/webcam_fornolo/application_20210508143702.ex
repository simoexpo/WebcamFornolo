defmodule WebcamFornolo.Application do
  ...
  def start(_type, _args) do
    ...
    port = String.to_intxeger(Map.get(System.get_env(), "PORT", @default_port))
    Logger.info("Starting server on port #{port}")

    username ="FornoloFoscam2"
    password ="Seragele01%"x

    session = RTSP.Session.start_link("rtsp://#{username}:#{password}@192.168.178.104:88")
    asd(session)
