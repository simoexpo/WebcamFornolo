defmodule WebcamFornolo.TestTransport do
  @moduledoc """
  ...
  @impl true
  def handle_info({:tcp, socket, data}, %State{caller: caller} = state) do
    IO.inspect(:gen_tcp.recv(socket, 8))
  ...
  defp execute_request(request, %State{connection: conn} = state) do
    with :ok <- :gen_tcp.send(conn, request) do
      {:ok, state}
    end
  end
end
